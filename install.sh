...
read -p "Public IP + PORT [ipv4:port] (example 4.3.2.1:54321):" ENDPOINT
if [ -z $ENDPOINT ]
then
echo "[#]Empty public Ip."
exit 1;
fi
...
if [ -z "$1" ]
  then 
    read -p "Enter VPN subnet, [default]: 10.50.0.1: " SERVER_IP
    if [ -z $SERVER_IP ]
...
read -p "Enter the ip address of the server DNS,  [default]: 8.8.8.8): " DNS
if [ -z $DNS ]
then DNS="8.8.8.8"
fi
echo $DNS > ./dns.var
...
read -p "interface name [default]: eth0): " WAN_INTERFACE_NAME
